...
class TestMetrics(unittest.TestCase):
    ...
    @patch("psutil.cpu_percent", side_effect=Exception("Test Exception"))  # Correctly mock the failure
    def test_metrics_failure(self, mock_cpu_percent):
        """Test error handling when retrieving metrics."""
        response = self.client.get('/metrics')
        self.assertEqual(response.status_code, 500, "Expected HTTP 500 when metrics retrieval fails")

        data = response.get_json()
        self.assertIn("error", data, "Response should contain an 'error' field")
        self.assertEqual(data["error"], "An error occurred while retrieving metrics.", "Error message mismatch")
